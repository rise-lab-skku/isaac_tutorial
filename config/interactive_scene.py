...
class InteractiveScene:
    ...
    def __init__(self, cfg: InteractiveSceneCfg):
        ...
        if assembly:
            # assembly
            base_robot_path = "/World/envs/env_0/Robot"
            attach_robot_path = "/World/envs/env_0/Gripper"
            base_robot_mount_frame = "/ee_link"
            attach_robot_mount_frame = "/bh_base_link"
            fixed_joint_offset = np.array([0.01,0.0,0.0])
            fixed_joint_orient = np.array([0.7071,0.0,0.7071,0.0])
            single_robot = False
            robot_assembler = RobotAssembler()
            arm_hand = robot_assembler.assemble_articulations(
                base_robot_path,
                attach_robot_path,
