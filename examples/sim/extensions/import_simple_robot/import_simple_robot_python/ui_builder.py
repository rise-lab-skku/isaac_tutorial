...
from omni.isaac.ui.element_wrappers import CollapsableFrame, StateButton, FloatField
from omni.isaac.ui.element_wrappers.core_connectors import LoadButton, ResetButton
from omni.isaac.ui.ui_utils import get_style
from omni.usd import StageEventType
from pxr import Sdf, UsdLux

from .scenario import SimpleRobotExampleScript
...
class UIBuilder:
    ...
    def build_ui(self):
        ...
        with self._robot_control_frame:
            with ui.VStack(style=get_style(), spacing=5, height=0):
                self.lin_vel_field = FloatField(label=f"Linear speed", tooltip="Set linear speed")
                self.ang_vel_field = FloatField(label=f"Angular speed", tooltip="Set angular speed")
                self.lin_vel_field.set_upper_limit(1.0)
                self.ang_vel_field.set_upper_limit(1.0)
                self.lin_vel_field.set_lower_limit(-1.0)
                self.ang_vel_field.set_lower_limit(-1.0)
    ...
    def _on_init(self):
        ...
        self._scenario = SimpleRobotExampleScript()
    ...
    def _on_post_reset_btn(self):
        ...
        """
        self.lin_vel_field.set_value(0.0)
        self.ang_vel_field.set_value(0.0)
    ...
    def _update_scenario(self, step: float):
        ...
        lin_vel = self.lin_vel_field.get_value()
        ang_vel = self.ang_vel_field.get_value()

        done = self._scenario.update(step, lin_vel, ang_vel)
        if done:
            self._scenario_state_btn.enabled = False
