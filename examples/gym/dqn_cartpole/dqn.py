...
class DQN:
    ...
    def update(self):
        ...
        loss = F.mse_loss(q_val, target)    # smooth_l1_loss, mse_loss
        # print("loss: ", loss)
    ...
    def select_action(self, obs, epsilon=0.0):
        ...
        act = coin.float() * rand_act + (1 - coin.float()) * true_act   # choose random action or policy action
        clip_act = 2 * (act - 0.5)  # maps to -1 to 1
        return clip_act

    def run(self):
        epsilon = max(0.01, 0.8 - 0.01 * (self.run_step / 20)) # linear annealing from 0.8 to 0.01 over 20k steps Epsilon으로 E-Greedy 탐색법에서 0.9부터 0.05까지 EPISODE를 반복할수록 낮추어가며 진행
